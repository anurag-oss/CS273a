...
def create_model(activation='relu'):
    # create model
    model = Sequential()
    model.add(Dense(14, input_dim=14, init='uniform', activation=activation))
...
# create model
model = KerasClassifier(build_fn=create_model,validation_split=0.1, verbose=1) 
print('Model created...')   
# define the grid search parameters
batch_size = [50, 100, 200, 300, 400]
epochs = [10, 20, 50]
activation = ['relu', 'tanh', 'sigmoid']
param_grid = dict(activation=activation,batch_size=batch_size, nb_epoch=epochs)
...
#    for row in params:
#        s=str(row['nb_epoch']) +','+row['activation']+','+str(row['batch_size'])+'\n'
