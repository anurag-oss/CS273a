...
model.add(Dense(7, init='lecun_uniform', activation='relu', W_constraint=maxnorm(3)))
model.add(Dropout(0.2))
...
predictions = model.predict(X_2K)

fh = open('stage1_output_from_nn.csv','w') 

for i,yi in enumerate(predictions):
    fh.write('{}\n'.format(predictions[i][0])) # output each prediction
